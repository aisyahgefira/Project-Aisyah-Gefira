...
cut_point=cutree(hc_complete,k=4) #memilih 2 cluster
data %>%
  mutate(cluster=cut_point) %>%
  group_by(cluster) %>%
  summarise(mean_year1=round(mean(year1),4),
            mean_year2=round(mean(year2),4),
            mean_year3=round(mean(year3),4),
            mean_year4=round(mean(year4),4),
            mean_year5=round(mean(year5),4))
